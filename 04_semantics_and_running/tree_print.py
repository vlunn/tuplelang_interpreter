        ...
        for name, val in vars(node).items():
            ...
            # An attribute containing a child list
                ...
                if val is None:
                    ...
                    assert hasattr(val,
                                   "__iter__"), "Children list is not a list!!!"
...
def treeprint_indent(node, outtype="unicode", label="", first_indent="",
                     indent=""):
...
def treeprint_dot(node, nodenum, nodecount):
    ...
    if not node:
        ...
        for name, value in childvars:
            ...
            print(dotnodeid(nodenum) + "->" + dotnodeid(
                childnum) + ' [label="' + name + '"]')
